...
/**
 * Middleware to allow both passengers and drivers
 * Must be used after authenticateToken
 */
const authenticateUser = (req, res, next) => {
    ...
    if (!['PASSENGER', 'DRIVER'].includes(req.user.role)) {
        return res.status(403).json({
            success: false,
            error: 'Access denied',
            message: 'Only passengers and drivers can access this resource'
        });
    }
 ...
 * For trips, either passenger owns it or driver is assigned to it
 */
const checkTripOwnership = (req, res, next) => {
    ...
    // We'll check ownership in the controller by querying the trip
